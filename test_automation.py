...
def _find_header_row(ws, max_scan_rows: int) -> int:
    ...
    for r in range(1, scan_limit + 1):
        ...
        texts = [v for v in values if isinstance(v, str) and v.strip() and len(v.strip()) <= 60]
        if len(texts) < 2:
            continue

        norms = {_normalize_header(v) for v in texts}

        # Exact match for our Excel structure: "Test Case ID" + "Input" + "Expected output"
        if "testcaseid" in norms and "input" in norms:
            return r
...
def _set_cell_value(ws, row: int, col: int, value):
    ...
    if not isinstance(cell, MergedCell):
        cell.value = value
        return
    # For merged cells, find the top-left cell of the merge range and write there
    for rng in ws.merged_cells.ranges:
        if rng.min_row <= row <= rng.max_row and rng.min_col <= col <= rng.max_col:
            top_left = ws.cell(row=rng.min_row, column=rng.min_col)
            top_left.value = value
            return
    # Fallback: unmerge and write directly
    cell.value = value
...
def _read_output(is_chat: bool, output_locator) -> str:
    # Try all methods to get output text - order matters
    # 1. Try evaluate to get value directly from DOM (works for read-only textareas too)
    try:
        v = output_locator.evaluate("(el) => el && ('value' in el ? el.value : el.innerText || el.textContent || '')")
        if v is not None:
            v = str(v).strip()
            if v:
                return v
        ...
        pass
    # 2. Try input_value (works for editable textareas)
    try:
...
def run_test():
        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            ...
            try:
                ...
                if action_locator:
                    ...
                    page.wait_for_timeout(500)  # Wait for button click to register

                # Initial wait for translation to start
                page.wait_for_timeout(max(0, int(args.wait_ms)))
                
                # Wait for visible content - retry with increasing waits
                actual_output = ""
                ...
                for i in range(tries):
                    ...
                    if current and current.strip():
                        # Make sure it's not the previous output
                        if not prev_output or current != prev_output:
                            actual_output = current
                            print(f"  Output received: {actual_output[:50]}...")
                            break
                    
                    # Wait before retry
                    if i < tries - 1:
                        wait_time = max(0, int(args.retry_wait_ms))
                        page.wait_for_timeout(wait_time)
                
                # If still empty, try one more time with longer wait
                if not actual_output:
                    page.wait_for_timeout(2000)
                    actual_output = _read_output(is_chat, output_locator)

                # Write to Excel - handle merged cells properly
                _set_cell_value(ws, row_index, actual_col_idx, actual_output)

                # Determine status
                if not actual_output or not actual_output.strip():
                    status = "NO OUTPUT"
                elif expected_sinhala:
                    # Compare outputs - exact match
                    if actual_output.strip() == expected_sinhala.strip():
                        status = "PASS"
                    else:
                        status = "FAIL"
                else:
                ...
                if args.save_every and int(args.save_every) > 0 and processed % int(args.save_every) == 0:
                    try:
                        wb.save(args.output)
                        print(f"  [Saved after {processed} tests]")
                ...
                try:
                    screenshot_path = f"error_row_{row_index}.png"
                    page.screenshot(path=screenshot_path)
                    print(f"  Screenshot saved: {screenshot_path}")
                except Exception:
                    pass
                
                # Write error details to Excel
                try:
                    _set_cell_value(ws, row_index, actual_col_idx, "")
                    _set_cell_value(ws, row_index, status_col_idx, f"ERROR: {error_msg[:50]}")
                except Exception:
                    pass
    ...
    try:
        ...
        print(f"Total tests processed: {processed}")
    except Exception as e:
        print(f"\nError saving output file '{args.output}': {e}")
        return
